...
For locus pair, we report three measures of overall linkage
...
:math:`S`
   ...
   better approximated using a randomization procedure. The empirical
   ...
   equilibrium. ( ``# permu`` indicates how many permutations were
   carried out). The :math:`p`-value is the fraction of permutations
   that results in values of `S` greater or equal to that observed. A
   :math:`p < 0.05` is indicative of overall significant LD.
