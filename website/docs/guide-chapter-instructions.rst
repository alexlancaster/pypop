...
degrees of freedom and associated ``p``-value for a number of classes of
...
-  **Common genotypes.**
   ...
   chi-square and ``p``-values for all observed genotypes with expected
   values GE ``lumpBelow``.
...
The ``p``-value in the last row of the output is the probability of
...
reported ``p``-values are one-tailed (against the alternative of
...
how many permutations were carried out). The ``p-value`` is the
