...
pypop = Analysis([INSTALLER + '/support/_mountzlib.py',
              INSTALLER + '/support/useUnicode.py',
              'pypop.py'],
             pathex=[])

pypop_pyz = PYZ(pypop.pure)

pypop_exe = EXE(pypop_pyz,
                pypop.scripts,
                exclude_binaries=1,
...
popmeta = Analysis([INSTALLER + '/support/_mountzlib.py',
              INSTALLER + '/support/useUnicode.py',
              'popmeta.py'],
             pathex=[])
popmeta_pyz = PYZ(popmeta.pure)

popmeta_exe = EXE(popmeta_pyz,
                  popmeta.scripts,
                  exclude_binaries=1,
                  name=os.path.join('buildstandalone', 'popmeta'),
                  debug=0,
                  strip=0,
                  console=1)
